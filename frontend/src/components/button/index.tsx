...
export const CButton = ({ style_type, icon, label, handler, size, disabled }: ICButton) => {
    ...
    return <button disabled={disabled} className={style_type.split(" ")[0] == "outline" ? outline_style : filled_style} onClick={handler}>{icon} {label}</button>
}